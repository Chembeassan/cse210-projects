...
namespace Farmer_s_Cooperative_Management_System
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Farmer farmer = RegisterFarmer();

            Console.WriteLine("Registration successful!");

            Dictionary<string, decimal> inventory = ManageInventory();

            Console.WriteLine("Inventory updated successfully!");

            Assets assets = ManageAssets(inventory);
            ...
            if (choice == 1)
            {
                FarmingSelection("Poultry Farming");
            }
            else if (choice == 2)
            {
                FarmingSelection("Fish Farming");
            }
            else if (choice == 3)
            {
                FarmingSelection("Vegetable Farming");
            }

            Console.WriteLine("\nThank you for using the Farmer's Cooperative Management System!");

            // Generate summary report
            var farmers = new List<Farmer> { farmer };
            var assetsList = new List<Assets> { assets };
            var farmDataAnalyzer = new FarmDataAnalyzer(farmers, assetsList);
            farmDataAnalyzer.GenerateSummaryReport();
        }

        static Farmer RegisterFarmer()
        {
            ...
            return new Farmer(name, specialization, contactNumber);
        }
        ...
        static Dictionary<string, decimal> ManageInventory()
        {
        ...
        static Assets ManageAssets(Dictionary<string, decimal> inventory)
        {
            Console.WriteLine("\nAssets Management");
            Console.WriteLine("-----------------");
            ...
            Console.WriteLine("------------------------");
            Console.WriteLine($"Total farmers registered: 1");
            ...
            return new Assets(farmingSpecialization, cash, land, goodwill + totalInventoryCost);
        }
        ...
        static void FarmingSelection(string farmingType)
        {
